...
void *mm__malloc(size_t size)
{
	...
	liballoc_lock();
		...
		remainder = tag->real_size - size - sizeof( struct boundary_tag ) * 2; // Support a new tag + remainder
...
void *malloc(size_t size) { return mm__malloc(size); }
